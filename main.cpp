...
std::vector<AdapterInfo> EnumerateAdapters(IDXGIFactory6* factory) {
...
// ------------------------------------------------------------
void RunBandwidthTest(const char* name, ID3D12Device* device, ID3D12CommandQueue* queue,
    ID3D12CommandAllocator* allocator, ID3D12GraphicsCommandList* list,
    ID3D12Fence* fence, HANDLE fenceEvent, UINT64& fenceValue,
    ComPtr<ID3D12Resource> src, ComPtr<ID3D12Resource> dst,
    size_t bufferSize, int copiesPerBatch, int batches)
{
    std::cout << name << "\n";
    PrintDivider();
    ...
    for (int i = 0; i < batches; i++) {
        ...
        queryReadback->Map(0, &range, reinterpret_cast<void**>(&ts));
        ...
        bandwidths.push_back((bytes / 1e9) / seconds);
...
// ------------------------------------------------------------
void RunLatencyTest(const char* name, ID3D12Device* device, ID3D12CommandQueue* queue,
    ID3D12CommandAllocator* allocator, ID3D12GraphicsCommandList* list,
    ID3D12Fence* fence, HANDLE fenceEvent, UINT64& fenceValue,
    ComPtr<ID3D12Resource> src, ComPtr<ID3D12Resource> dst, int iterations)
{
    std::cout << name << "\n";
    PrintDivider();
...
// ------------------------------------------------------------
void RunCommandLatencyTest(ID3D12CommandQueue* queue, ID3D12CommandAllocator* allocator,
    ID3D12GraphicsCommandList* list, ID3D12Fence* fence, HANDLE fenceEvent,
    UINT64& fenceValue, int iterations)
{
...
int main() {
    ...
    constexpr size_t LargeSize = 256ull * 1000 * 1000;
    constexpr size_t SmallSize = 1;
    constexpr int CommandIters = 100000;
    constexpr int TransferIters = 10000;
    ...
    if (adapters.empty()) { std::cerr << "No GPUs found\n"; return -1; }

    std::wcout << L"Available GPUs:\n";
    for (size_t i = 0; i < adapters.size(); ++i) {
        std::wcout << L"[" << i << L"] " << adapters[i].desc.Description
            << L" (VRAM " << adapters[i].desc.DedicatedVideoMemory / (1000 * 1000 * 1000) << L" GB)\n";
    }

    PrintDivider();
    std::wcout << L"Select GPU: ";
    size_t selected = 0;
    while (true) {
        std::wcin >> selected;
        if (!std::wcin.fail() && selected < adapters.size()) break;
        std::wcin.clear();
        std::wcin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        std::wcout << L"Invalid selection, try again: ";
    }

    auto adapter = adapters[selected].adapter;
    PrintDivider();
    std::wcout << L"Using GPU: " << adapters[selected].desc.Description << L"\n";
    PrintDoubleDivider();
